...
# Counts of Prospects by Contact Month
ggplot(data = all_data, aes(x = LastContactMonth)) + 
  geom_bar(position = "dodge", stat = "count")  +
  ggtitle("Counts by Contact Month") +
...
pros_clients <- m_test[order(-rank(m_test$Accept_Offer)), ]
pros_clients <- pros_clients[order(-pros_clients$Accept_Prob), ]
pros_clients <- head(pros_clients, 100)

leads <- pros_clients[, c("HHuniqueID", "Accept_Offer", "Accept_Prob")]

# Send the results to Excel
output_file <- "A2 - Alvaro Ronquillo - Leads.csv"
write.csv(leads, file = output_file, row.names = FALSE)
...
# HHInsurance relationship with Age
ggplot(data = subset(pros_clients, HHInsurance == 0), aes(x = Age)) + 
  geom_histogram(binwidth = 1, color = "black", fill = "steelblue") +
  xlab("Age") + ylab("Count") +
  ggtitle("Age Distribution for Leads with No Insurance") +
  theme_bw()


luxury <- c("Acura", "Audi", "BMW", "Cadillac", "Infiniti", "Land Rover", "Lexus", "Lincoln", "Lotus", "Maserati", "Mercedes-Benz")
regular <- c("Buick", "Chevrolet", "Chrysler", "Daewoo", "Dodge", "Eagle", "Fiat", "Ford", "GMC", "Honda", "Hyundai", "Isuzu", "Jeep", "Kia", "Mazda", "Mercury", "Mitsubishi", "Nissan", "Oldsmobile", "Pontiac", "Saab", "Saturn", "Subaru", "Suzuki", "Toyota", "Volkswagen", "Volvo")

get_category <- function(brand) {
  if (brand %in% luxury) {
    return("Luxury")
  } else if (brand %in% regular) {
    return("Functional")
  } else {
    return("")
  }
...
pros_clients$Category <- sapply(pros_clients$carMake, get_category)


# Counts of Prospects by CarMaker
ggplot(data = pros_clients, aes(x = Category)) + 
  geom_bar(position = "dodge", stat = "count")  +
  ggtitle("Counts by Car Category") +
  scale_fill_manual(values = c("darkgreen", "navy")) + 
  ylab("Count") + 
  xlab("Category")
